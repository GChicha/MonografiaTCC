...
tanto  do de  médias movéis.  Para essa  tarefa \citeonline{vinay}  apresenta a
\autoref{eq:coef}  que  serve  para  determinação  dos  coeficientes  de  ambos
processos.
...
\begin{figure}[ht]\label{fig:perceptron}
    \centering
    \caption{Representação gráfica de um \textit{perceptron}}
    \includegraphics[width=.5\linewidth]{images/perceptron.png}
    \source{\citeonline{haykin}}
\end{figure}
...
\begin{equation}\label{eq:backpropagation}
    w_{n+1} = w_n - \alpha \frac{\theta E(x,w_t)}{\theta w_t}
\end{equation}
