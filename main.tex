...
Durante a análise outra função que será relevante é a autocorrelação parcial,
sua definição segundo \citeonline{cryer} é dado em \autoref{eq:facp}, onde
...
podendo contar com a ajuda de  outros testes para validação da estacionariedade
da série.

%  FIXME Corrigir  inicio  O  método proposto  por  \citeonline{box} observa  o
...
Quanto  a  utilização   de  redes  neurais  podemos  observar   o  trabalho  de
\citeonline{zhangbco}.  Neste trabalho  foi observado  o desempenho  do uso  de
redes neurais  utilizando o algorítimo de  retro propagação e outro  baseado em
algorítimos naturais. A série utilizado foi  a do índice S\&P500, foi comparada
...
na de  saída. O modelo proposto  utilizando o algorítimo neural  teve resultado
superior para todas as analises.
...
servindo como um  indicador geral do comportamento do mercado.  Logo foi obtido
...
avaliação.  Especificamente  para  os  modelos  probabilísticos  foi  utilizado
a    biblioteca    statsmodels\footnote{\url{https://www.statsmodels.org}}    e
...
\autoref{fig:traintest} vemos estes conjuntos.
...
Após a análise foi encontrado os modelos que mais se adequam a série e validada
...
Já na análise dos gráficos da  série, na \autoref{fig:sp500}, fica evidente o a
existência de  tendência, permitindo concluir  que a série não  é estacionaria.
Conclui-se  isso  também a  partir  da  análise da  \autoref{fig:acfsp500}  que
demonstra a  existência de um  grande numero  de correlações positivas  fora do
...
diferenciação. Temos na \autoref{fig:sp500diff}  a primeira diferenciação. Pela
observação  da função  autocorrelação  na \autoref{fig:acf_diff_sp500}  podemos
...
Quanto a sazonalidade, podemos observar a  existência de um padrão que remete a
...
Tendo  sido  realizada  a  análise  pode-se concluir  que  o  modelo  ARMA  sem
integração não seria suficiente, assim o  parâmetro $d$ do modelo ARIMA será de
ordem 1,  devido a ter  sido necessária uma  diferenciação para tornar  a serie
estacionaria.
...
\autoref{fig:sp500pacf}. Segundo o modelo de \citeonline{box} temos de analisar
...
Para podermos concluir como um bom modelo, temos ainda que analisar se a função
de autocorrelação e autocorrelação parcial  dos resíduos apresenta a existência
...
\begin{figure}[ht]
    \caption{Função de autocorrelação e autocorrelação parcial dos residuos do
...
\begin{table}[ht]
\centering
\caption{Parâmetros utilizados da busca em grade}\label{tab:gridsearch}
\begin{tabular}{l l}
...
a  aplicação  da regularização  foi  o  rápido  \textit{overfit} nos  dados  de
...
foi com duas camadas, sendo a primeira  camada com 13 neurônios e a segunda com
somente um neurônios já  que esta é a camada de saída. A  função de ativação de
...
similar em seu  desempenho. Vale notar que os resultados  não foram expressivas
na conclusão de  superioridade de um modelo. Quanto a  métrica utilizada para a
avaliação, o modelo ARIMA teve resultado superior em uma ordem ínfima, 0,6\% em
relação ao modelo de redes neurais.

O  modelo probabilísticos  obtido foi  um modelo  hibrido ARMA,  de ordem  5 do
processo autorregressivo e ordem 7 para o processo de medias móveis. Também foi
necessária uma diferenciação  já que a série apresentou tendencia,  e por conta
disso,  foi  necessária  a  diferenciação  para  para  obtenção  de  uma  série
estacionaria, característica essa requerido pelo modelo ARMA\@.

Para obtenção  do modelo de  redes neurais a mudança  dos dados se  resumiram a
realizar  de um  dimensionamento para  um intervalo  de valores,  sem se  fazer
necessário maior avaliação do comportamento, já que segundo \citeonline{haykin}
algorítimos de aprendizado  são ditos dirigido por dados. No  entanto durante a
...
meio desse processo se identificou uma rede  neural de duas camadas, sendo a de
entrada com 13 unidades de  processamento, \textit{perceptron}, e a segunda com
um único neurônio, sendo  essa camada a de saída e  representando a previsão um
passo a diante.
